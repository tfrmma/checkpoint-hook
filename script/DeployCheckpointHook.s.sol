...
contract DeployCheckpointHook is Script {
    ...
    function run() external returns (CheckpointHook hook) {
        ...
        bytes memory constructorArgs = abi.encode(poolManager, blockNumberOffset, governance, treasury);
        ...
        vm.startBroadcast();
        hook = new CheckpointHook{salt: salt}(poolManager, blockNumberOffset, governance, treasury);
        vm.stopBroadcast();
